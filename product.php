...
$statement = $pdo->prepare("SELECT * FROM tbl_product_weight WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
foreach ($result as $row) {
...
if(isset($_POST['form_add_to_cart'])) {
    ...
    if(isset($_SESSION['cart_p_id']))
    {
        ...
        $i=0;
        foreach($_SESSION['cart_weight_id'] as $key => $value) 
        {
            $i++;
            $arr_cart_weight_id[$i] = $value;
        }


        $added = 0;
        if(!isset($_POST['weight_id'])) {
            $weight_id = 0;
        } else {
            $weight_id = $_POST['weight_id'];
        }
        for($i=1;$i<=count($arr_cart_p_id);$i++) {
            if( ($arr_cart_p_id[$i]==$_REQUEST['id']) && ($arr_cart_weight_id[$i]==$weight_id) ) {
                $added = 1;
                break;
            }
        }
        if($added == 1) {
           $error_message1 = 'This product is already added to the shopping cart.';
            ...
            if(isset($_POST['weight_id'])) {
                $weight_id = $_POST['weight_id'];
                $statement = $pdo->prepare("SELECT * FROM tbl_weight WHERE weight_id=?");
                $statement->execute(array($weight_id));
                $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
                foreach ($result as $row) {
                    $weight = $row['weight'];
                }
            ...
            $_SESSION['cart_weight_id'][$new_key] = $weight_id;
            $_SESSION['cart_weight'][$new_key] = $weight;
    ...
    else
    {
        ...
        $_SESSION['cart_weight_id'][1] = $weight_id;
        $_SESSION['cart_weight'][1] = $weight;
                            ...
                            <div class="p-quantity">
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select name="weight_id" class="form-control select2" style="width:auto;">
                                            <?php
                                            $statement = $pdo->prepare("SELECT * FROM tbl_weight");
                                            $statement->execute();
                                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($result as $row) {
                                                if(in_array($row['weight_id'],$weight)) {
                                                    ?>
                                                    <option value="<?php echo $row['weight_id']; ?>"><?php echo $row['weight']; ?></option>
                                                    <?php
                                                }
                                            }
                                            ?>
