...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['district_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_district WHERE district_name=? and district_name!=?");
    	$statement->execute(array($_POST['district_name'],$current_district_name));
    ...
    if($valid == 1) {    	
		...
		$statement = $pdo->prepare("UPDATE tbl_district SET district_name=? WHERE district_id=?");
		$statement->execute(array($_POST['district_name'],$_REQUEST['id']));
