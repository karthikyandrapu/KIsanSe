...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		if(isset($_POST['weight'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_weight WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['weight'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_weight (weight_id,p_id) VALUES (?,?)");
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="weight[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_weight ORDER BY weight_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);			
									foreach ($result as $row) {
										if(isset($weight_id)) {
											if(in_array($row['weight_id'],$weight_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
										...
										?>
										<option value="<?php echo $row['weight_id']; ?>" <?php echo $is_select; ?>><?php echo $row['weight']; ?></option>
										<?php
									}
									?>
								</select>
