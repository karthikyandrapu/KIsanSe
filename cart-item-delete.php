...
for($i=1;$i<=count($arr_cart_p_id);$i++) {
    if( ($arr_cart_p_id[$i] == $_REQUEST['id']) && ($arr_cart_weight_id[$i] == $_REQUEST['weight']) ) {
        continue;
    } else {
        $_SESSION['cart_p_id'][$k] = $arr_cart_p_id[$i];
        $_SESSION['cart_weight_id'][$k] = $arr_cart_weight_id[$i];
        $_SESSION['cart_weight'][$k] = $arr_cart_weight[$i];
